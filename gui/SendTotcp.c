...
#define  HEART         1   					//1 时发送心跳有返回     0 时发送心跳无返回
...
int sockfd = -1; 						//Socket
int  RI = 0; 						//接收数据数
extern FILE *canshu; 					//文件
extern FILE *BlackFile;					//文件
extern FILE *ParaFile;
extern FILE *ParaFileBuf;
extern FILE *Rechargefile;
FILE *PRINTFile;
extern unsigned char TcpIpBuf[35];     			//IP 地址
extern CardInform CardLan;             			//卡片格式缓冲区
extern RecordFormat SendDataK;         			//发送数据
extern LongUnon DevNum;                			//机号
...
char HeartData[10]= {"ZZXXCCPK"}; 		//心跳包
unsigned char G_START[8];  				// 协议字
unsigned char G_STARTW[8]; 				// 协议字
/********************** Flag  **********************/
extern volatile unsigned char SendDataTimes;
extern unsigned char COMNET;  // 1: 有线TCP/IP   2: 天线WIFI     3: 天线CDMA   4: 无线GPRS
...
extern LongUnon CodeNum;       	//数据上传
extern LongUnon SaveNumBs;          //数据
...
/*
*************************************************************************************************************
- 函数名称 : unsigned char LenDataCmd(unsigned char *DataLen)
- 函数说明 : 模块返回数据分析
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : unsigned char SendHandData(unsigned char *send55AA)
- 函数说明 : 发送数据  不包含协议
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : unsigned char CheakIsConnectToInternel(void)
- 函数说明 : 发送数据  不包含协议
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : char GprsCmd(unsigned char *SndDa)
- 函数说明 : 发送数据      包含协议
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : void SendCmdPc(INT8U *Date,INT8U Len)
- 函数说明 :  发送数据      包含协议
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : void SendCmdPcEight(unsigned char *Date,unsigned char Len,unsigned char icount)
- 函数说明 : 完全采集
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/*
*************************************************************************************************************
- 函数名称 : void All_Cmd_Snd(unsigned char *Data)
- 函数说明 : 命令参数数据回复
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
    ...
    if(status == MI_OK)
    {
       ...
       case 0x28:							//读取终端时间
        ...
        case 0x38:							//设置用户服务器
            ...
            break;
        case 0x39:							//设置用户扇区
            memcpy(&LanSec,Data+4,8); // 1--16为M1卡扇区
            memcpy(LanSec.ADFNUM,Data+20,2); // 17-- 为CPU卡ADF
            /// while (FileOpenFlag == 0);
            //    FileOpenFlag = 0;
		...
		case 0x78:
			if(LocalCRC16 == UpCRC16)
			{
				...
				*(Data+6+BuffWord.word) = 0;
				...
				system("sync;");
				sleep(5);
				if(Data[6]==0x42 && Data[7]==0x6C)//Bl黑名单
				{
					ReloadBlackListBuff();
				}
				else if(Data[6]==0x42 && Data[7]==0x61)//Ba卡柄
				{
				//	InitCardBin(0);
				}
				else
...
//			Clear_bStartUpLoad();
		...
		case 0xb5: //数据出错处理，修复数据input/output error
...
/*
*************************************************************************************************************
- 函数名称 : int write_datas_tty(int sockxx, unsigned char *buffer, int buf_len)
- 函数说明 : 写 SOCKET 数据 及超时处理
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
*/
...
/*
*************************************************************************************************************
- 函数名称 : unsigned char read_datas_tty(int fd,unsigned char *buffer)
- 函数说明 : 读SOCKET数据
- 输入参数 : 无
- 输出参数 : 无
*************************************************************************************************************
...
/****************************************************************************
* 名称：int InquireSIMIMSI(char *dev)
* 功能：初始化M26
* 入口参数：无
* 出口参数：无
****************************************************************************/
...
int InquireSIMIMSI(void)
{
	...
	{  
		...
		if (cnt >= 30)
		{
			...
			else if ((strchr(RX_buff, '0') !=  NULL) && (strchr(RX_buff, '3') !=  NULL))    //无效电话卡
			{
				printf("error 00无效电话卡\n");
				return 1;
			}
...
LOOP1:
	...
	for(;;)
	{
    	if(ConnectFlag == 0)  //接收到数据
    	{
        	ret = read_datas_tty(sockfd,rebuf);				//读消费数据
    		if(ret  == 0)
    		{
    				Heart = 0;
    				heartlen = 0;
...
Line_parse:
	...
	#endif
						status = 0;
						memset(dppd,0xff,sizeof(G_STARTW));// 清除返回的标记
			...
			{
				Heart = 0;
				memset(dppd,0xff,sizeof(HeartData));// 清除返回的标记
				Netstatus = 1;                      //接收到心跳包才置升级标志，表示在空闲状态下才进行文件升级 
			}
	#endif
			...
			if(dpp != NULL)
			{
				...
				if((lend > 0)&&(lend <= 200))
				{
					...
					if(((lend+8)*2 == strlen(RcvCmd)))  //收到完整的贞数据
					{
	#if GPRSPR
						printf("------------>");
						printf(RcvCmd);		//收到完整的贞数据
						printf("\n");		//收到完整的贞数
	#endif
						memcpy(G_START,dpp+5,8);	//协议字
						memset(dpp,'f',5);
        ...
        /*		else
        		...
        		*/
                ...
                dpp = strstr(Gprs_xf,FILEHEAD);        //下载文件命令  
                if(dpp != NULL)
                {
                    Heart = 0;
					heartlen = 0;
                    ...
                    { 
                        ...
                        if(((lend+9)*2 == strlen(RcvCmd)))  //收到完整的贞数据
                        {
	#if GPRSPR
                            printf("FIEL------------>"); 
                            printf(RcvCmd);		//收到完整的贞数据
                            printf("\n");		//收到完整的贞数
	#endif
                            memcpy(G_START,dpp+4,8);	//协议字
                            memset(dpp,'f',5);
                            memset(RCmdata,0,sizeof(RCmdata));
                            AHLen = ascii_2_hex(RcvCmd+8,RCmdata,(lend+5)*2);  //将数据从ASC2转换成HEX
                            //printf("转码后的数据长度:%d\n",AHLen);
	#if UPDEG 
		...
		DebugPrintf("status = %u\n",status);
			...
			DebugPrintf("status = %u\n",status);
        			...
        			if(status == 0)
        			{
            			...
            			//Display_signal(ConnectFlag);去掉此行防止程序死机
        			}
        		}
    		}
    	}
